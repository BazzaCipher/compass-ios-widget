...
2. Create a new script, and copy the code in `CompassWidget.js` into it. Make sure this script is called `CompassWidget`.
3. Create a new script, and copy the code in `CompassAPI.js` into it. Make sure this script is called `CompassAPI`.
4. Download `Compass_Resources.zip` into "On My iPhone/iPad" and decompress it.
5. Go to Scriptable Settings > File Bookmarks and select "Pick Folder". Choose the Compass_Resources, keeping the bookmark name as `Compass_Resources.`

## Setting up authentication
1. Log in to your Compass account as normal in **a private browsing window in Chrome or Firefox.** You'll need to use a private window so you don't accidentally log out of the session.
2. Open the developer tools and switch to the "Console" tab.
3. Paste the following code and press enter (this extends your session so that it won't expire automatically). Note down the number that is output in the console - it is your User ID which you'll need later:

```js
Compass.postWithCallback("/Services/Mobile.svc/UpgradeSamlSession", {}, d => console.log(Compass.organisationUserId))
```

4. Follow the below steps depending on which browser you have:
   - Chrome:
...
5. Open `CompassWidget` in the Scriptable editor and change these values:
   1. `yourdomain.compass.education` should be changed to your school's Compass domain.
   2. `userId: 0` should be changed to the ID you got in Step 3.
   3. `sessionToken: ""` should be changed to the session token you copied in Step 4.
